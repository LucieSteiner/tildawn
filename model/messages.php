...
function countActiveMessages(){
	$link = connect();
    $res = mysqli_query($link, 'select count(*) as nb from messages where `status`= 1;');

    return fetch_result($res);
}
function getAllActiveMessages(){
	$link = connect();
    $res = mysqli_query($link, 'select * from messages where `status`= 1;');
...
function toggleMessageStatus($messageId){
    $link = connect();
    mysqli_query($link, 'update messages set `status`=NOT(`status`) where `id`='. $messageId .';') or die(mysqli_error($link));
...
function deleteMessage($messageId){
    $link = connect();
    mysqli_query($link, 'delete from messages where `id`='. $messageId .';')or die(mysqli_error($link));
...
function createNewMessage($title, $text){
    $link = connect();
    mysqli_query($link, 'insert into messages(`title`, `text`, `status`) values("'. $title .'", "'. $text .'", 0);') or die(mysqli_error($link));
...
function getAllAlerts(){
    ...
    $link = connect();
    $res = mysqli_query($link, 'select `id`, `title`, `text`, `status` from alerts;');

    return fetch_result($res);
}
function countActiveAlerts(){
	$link = connect();
    $res = mysqli_query($link, 'select count(*) as nb from alerts where `status`= 1;');

    return fetch_result($res);
}
function getAllActiveAlerts(){
	$link = connect();
    $res = mysqli_query($link, 'select * from alerts where `status`= 1;');
...
function toggleAlertStatus($alertId){
    $link = connect();
    mysqli_query($link, 'update alerts set `status`=NOT(`status`) where id ='. $alertId .';');
...
function createNewAlert($title, $text){
    $link = connect();
    mysqli_query($link, 'insert into alerts(`title`, `text`) values("'. $title .'", "'. $text.'");');
